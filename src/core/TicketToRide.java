	...
	public void onPlayerEvent(PlayerEvent e) {
		...
		if (eventID == -1) {
			...
			if (eventID % 10 == 8) {
				...
				if (rail.isDouble() && rail.getOwnerName(1) != null
						&& rail.getOwnerName(1).equals(currentPlayer.getName())) {
				...
				railNum = 1;
				if (rail.isDouble() && rail.getOwnerName(0) != null
						&& rail.getOwnerName(0).equals(currentPlayer.getName())) {
		...
		for (Player P : players) {
			...
			if (paths[index] == longestPath) {
				...
				for (Rail r : P.getPath()) {
					...
					if (railIndex == -1)
						railIndex = graph.indexList().indexOf(graph.getInverse(r));
//					railIndex*=10;
